...
export function validateRawExtractedPdfText(text: string): ValidationResult {
  const trimmed = text.trim();
  const estimatedTokens = estimateTokens(trimmed);
  ...
  if (trimmed.startsWith("%PDF-")) {
    return {
      ok: false,
      estimatedTokens,
      reason: "Raw extraction still contains PDF bytes",
    };
  }
  ...
  if (suspiciousMarkerCount >= 8) {
    return {
      ok: false,
      estimatedTokens,
      reason: "Raw extraction still contains PDF internals",
    };
  }
  ...
  if (printableRatio < 0.75 || alphaNumericRatio < 0.1) {
    return {
      ok: false,
      estimatedTokens,
      reason: "Raw extraction looks too binary to trust",
    };
  }
  ...
  if (trimmed.length < 200) {
    return {
      ok: false,
      estimatedTokens,
      reason: "Raw extraction was too short to salvage",
    };
  }
  ...
  if (estimatedTokens > MAX_DOCUMENT_ANALYSIS_TOKENS) {
    return {
      ok: false,
      estimatedTokens,
      reason: `Raw extraction exceeded the analysis budget (${estimatedTokens} tokens estimated)`,
    };
  }
