...
export async function discoverScholarlySources({
  ...
  const searchQueries = uniqueQueries([trimmedQuery, ...queryVariations]).slice(0, 3);
  const perQueryResults = Math.max(1, Math.ceil(numResults / searchQueries.length));
  ...
  if (openAlexApiKey?.trim()) {
    for (const searchQuery of searchQueries) {
      tasks.push(
        ...
        }).then((works) => works.map(normalizeOpenAlexWork).filter(isCandidate))
      );
    }
  }
  ...
  for (const searchQuery of searchQueries) {
    tasks.push(
      semanticScholarSearch({
        ...
        result.papers.map(normalizeSemanticScholarPaper).filter(isCandidate)
      )
    );
  }
  ...
  if (!skipArxiv) {
    ...
    tasks.push(
      ...
      }).then((result) => result.papers.map(normalizeArxivPaper).filter(isCandidate))
    );
  }
  ...
  return mergeAndRankCandidates(candidates)
    .slice(0, numResults)
...
function mergeAndRankCandidates(candidates: NormalizedCandidate[]) {
